/*Les tas de scala ne sont pas bien donc j'ai du prendre autre chose :
n'ayant pas eu le temps de bien les implémenter j'ai pris cette implémentation : https://github.com/tomtung/tifmo/blob/master/src/main/scala/mylib/misc/FibonacciHeap.scala
*/
package mylib.misc {
...
class FNode[T](val data: T, origv: Double) {
    private[misc] var key = origv

    def value = key

    private[misc] var parent = null: FNode[T]
    private[misc] var child = null: FNode[T]
    ...
    private[misc] def cascadingCut(min: FNode[T]) {
      val z = parent
      if (z != null) {
        if (mark) {
          z.cut(this, min)
    ...
    private[misc] def cut(x: FNode[T], min: FNode[T]) {
    ...
    private[misc] def link(prt: FNode[T]) {
      left.right = right
      right.left = left
      parent = prt
      if (prt.child == null) {
